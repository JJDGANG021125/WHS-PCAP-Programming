...
void got_packet(
    ...
    /* 캡처된 데이터가 IP Header 전체를 포함하는지 확인 */
    if (header->caplen <
        (bpf_u_int32)
    ...
    /* TCP Header 최소 길이는 20 bytes */
    if (tcp_header_len < 20) {
        return;
    }


    /* 캡처 데이터가 TCP Header 전체를 포함하는지 확인 */
    if (header->caplen <
        (bpf_u_int32)
        (ETHER_HDR_LEN +
         ip_header_len +
