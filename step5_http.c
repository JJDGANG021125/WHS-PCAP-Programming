...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    headers_len = ETHER_HDR_LEN + ip_header_len + tcp_header_len;
    ...
    if (payload_len > (int)header->caplen - headers_len) {
        payload_len = (int)header->caplen - headers_len;
    }
    ...
    if (payload_len > 0 && (src_port == 80 || dst_port == 80)) {
        ...
        printf("[HTTP Message]\n");
        printf("No HTTP payload in this TCP packet.\n");
    }
